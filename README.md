...
The computed spectrums are stored in the three files below as follows. For any integer `i` in the number of lines in the csv below, the `i`-th line of the `SUMMARY` file provides the parameters q,n1,...,nm of the tensor space *(of size n1 x ... x nm over Fq)* such that the `i`-th line of the `EIGENVALUE` file contains the different eigenvalues of the multilinear forms graph spectrum for such parameters, and whose multiplicities can be found (with the same order) in the `MULTIPLICITIES` file. More precisely, if that spectrum has been computed, there exists a line $i$ such that the spectrum $\theta_0 > \theta_1 > ... > \theta_r$ with respective multiplicities $m_0,m_1,...,m_r$ of the multilinear forms graph $Cay(\mathbb F_q^{n_1 \times ... \times n_m}, \mathbb S_{n_1 \times ... \times n_m})$ is stored as follows.
+ The file [`DATASET_EIGENVALUES_KnownSpectra.csv`](EigenvaluesComputation/DATASET_EIGENVALUES_KnownSpectra.csv/) contains lines of the form $\theta_0$, $\theta_1$, ... , $\theta_r$
+ The file [`DATASET_MULTIPLICITIES_KnownSpectra.csv`](EigenvaluesComputation/DATASET_MULTIPLICITIES_KnownSpectra.csv/) contains lines of the form $m_0$, $m_1$, .... , $m_r$
+ The file [`DATASET_SUMMARY_KnownSpectra.csv`](EigenvaluesComputation/DATASET_SUMMARY_KnownSpectra.csv/) contains lines of the form $q$, $n_1$, ... , $n_m$.
